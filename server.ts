...
async function startServer() {
  ...
  app.get("/api/ml/categories", async (req, res) => {
    ...
    try {
      ...
      if (mainError) {
        console.warn("[Categories] Warning fetching main categories (might not exist):", mainError.message);
        // Silent recovery for categories
      ...
      if (subError) {
        console.warn("[Categories] Warning fetching subcategories (might not exist):", subError.message);
      ...
      console.error("[Categories] Global error:", err);
      res.json({ categories: [], subcategories: [] });
    }
  });
  ...
  app.post("/api/products/bulk", async (req, res) => {
    ...
    try {
      let currentProducts = [...formattedProducts];
      let { data, error } = await supabase
        .from('products')
        .upsert(currentProducts, { onConflict: 'code,company_id' })
        .select();

      // Exhaustive Resilience for bulk
      let retryCount = 0;
      const MAX_RETRIES = 15; // Limit retries to avoid infinite loops

      while (error && error.code === 'PGRST204' && retryCount < MAX_RETRIES) {
        const missingMatch = error.message.match(/Could not find the '(.+?)' column/);
        if (missingMatch && missingMatch[1]) {
          const missingField = missingMatch[1];
          console.warn(`[Bulk Products] Exhaustive Retry ${retryCount + 1}: Stripping column '${missingField}'`);
          
          currentProducts = currentProducts.map(p => {
            const newP = { ...p };
            delete (newP as any)[missingField];
            return newP;
          });

          if (currentProducts.length === 0 || Object.keys(currentProducts[0]).length === 0) break;

          const retryRes = await supabase.from('products').upsert(currentProducts, { onConflict: 'code,company_id' }).select();
          data = retryRes.data;
          error = retryRes.error;
          retryCount++;
        } else {
      ...
      if (error) {
        console.error("[Bulk Products] Final Error:", error);
        return res.status(500).json(error);
      }
      res.json(data);
    } catch (err: any) {
      console.error("[Bulk Products] Unexpected error:", err);
      res.status(500).json({ error: err.message });
    }
  });
