...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    { name: 'Reportes', path: '/reports', icon: AlertCircle },
  ];
  ...
  const adminItems = [
    ...
    { name: 'Reportes Admin', path: '/admin/reports', icon: AlertCircle },
  ];
...
const Products = () => {
  ...
  const handleImportXls = () => {
    ...
    setTimeout(() => {
      const mockImported = [
        { id: 'MLA1', title: 'Producto A', price: 100, status: 'pendiente' },
        { id: 'MLA2', title: 'Producto B', price: 200, status: 'pendiente' },
        { id: 'MLA3', title: 'Producto C', price: 300, status: 'pendiente' },
      ];
      setSyncItems(mockImported);
      setImporting(false);
      alert('Archivo .xls importado con éxito. Ahora puedes sincronizar a ML.');
    }, 1500);
  };

  const syncToMeli = async () => {
    if (!profile?.meli_connected) {
      alert('Debes conectar tu cuenta de Mercado Libre primero.');
      return;
    }
    
    setLoading(true);
    const updatedItems = [...syncItems];
    
    for (let i = 0; i < updatedItems.length; i++) {
      try {
        // Simulate API call to ML
        await new Promise(resolve => setTimeout(resolve, 800));
        updatedItems[i].status = 'actualizado';
      } catch (err: any) {
        updatedItems[i].status = 'error';
        updatedItems[i].errorDetail = err.message || 'Error desconocido';
      }
      ...
      <header className="flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <button 
            ...
            disabled={importing}
            className="bg-zinc-900 text-white px-4 py-2 rounded-xl flex items-center gap-2 transition-all disabled:opacity-50 shadow-sm"
          >
            <Upload size={16} />
            Importar .xls
          </button>
          <button 
            onClick={syncToMeli}
            disabled={loading || syncItems.length === 0}
            className="bg-blue-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 transition-all disabled:opacity-50 shadow-sm"
          >
      ...
      {syncItems.length > 0 && (
        <div className="bg-white border border-zinc-200 rounded-3xl overflow-hidden shadow-sm">
          <div className="p-6 border-b border-zinc-100">
            <h3 className="font-bold text-black">Estado de Sincronización</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                  ...
                  <tr key={idx} className="hover:bg-zinc-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${
                        item.status === 'actualizado' ? 'bg-emerald-100 text-emerald-600' :
                    ...
                    <td className="px-6 py-4 text-xs text-zinc-500">{item.errorDetail || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const Reports = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ nombre: '', empresa: '', motivo: '' });
    } catch (err: any) {
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <div className="bg-white border border-zinc-200 rounded-3xl p-8 shadow-sm">
        <h2 className="text-2xl font-bold text-black mb-6">Enviar Reporte</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input 
              ...
              className="w-full bg-zinc-50 border border-zinc-200 rounded-2xl px-4 py-3 text-black focus:outline-none focus:border-blue-500 transition-all"
              placeholder="Tu nombre completo"
            />
          </div>
          <div>
            <label className="block text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2">Empresa</label>
            <input 
              type="text" 
              required
          ...
          <div>
            <label className="block text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2">Motivo</label>
            <textarea 
              required
              value={formData.motivo}
              onChange={(e) => setFormData({...formData, motivo: e.target.value})}
              className="w-full bg-zinc-50 border border-zinc-200 rounded-2xl px-4 py-3 text-black focus:outline-none focus:border-blue-500 transition-all h-32 resize-none"
              placeholder="Describe el problema o motivo del reporte..."
            />
          </div>
          <button 
            type="submit"
            disabled={loading}
            className="w-full bg-black text-white font-bold py-4 rounded-2xl hover:bg-zinc-800 transition-all disabled:opacity-50"
          >
...
const AdminReports = () => {
  ...
  const handleAction = async (id: string, action: 'eliminar' | 'solucionado') => {
    try {
      if (action === 'eliminar') {
        const { error } = await supabase
          .from('reports')
      ...
      alert(`Reporte ${action === 'eliminar' ? 'eliminado' : 'marcado como solucionado'}.`);
    } catch (err: any) {
  ...
  return (
    <div className="p-8 space-y-8">
      <header>
        <h2 className="text-3xl font-bold text-black">Gestión de Reportes</h2>
        <p className="text-black/60">Revisa y gestiona los reportes enviados por las empresas.</p>
      </header>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-zinc-50 text-zinc-500 text-xs uppercase tracking-wider">
                <th className="px-6 py-4 font-medium">Fecha</th>
                <th className="px-6 py-4 font-medium">Empresa / Usuario</th>
            ...
            <tbody className="divide-y divide-zinc-100">
              {reports.map((report) => (
                <tr key={report.id} className="hover:bg-zinc-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <p className="text-sm font-bold text-black">{report.empresa}</p>
                    <p className="text-xs text-zinc-500">{report.nombre}</p>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <button 
                      onClick={() => handleAction(report.id, 'solucionado')}
                      className="bg-emerald-100 text-emerald-600 px-3 py-1 rounded-lg text-xs font-bold hover:bg-emerald-200 transition-all"
                    >
                      Solucionado
                    </button>
                    <button 
                      onClick={() => handleAction(report.id, 'eliminar')}
                      className="bg-red-100 text-red-600 px-3 py-1 rounded-lg text-xs font-bold hover:bg-red-200 transition-all"
                    >
                      Eliminar
                    </button>
                  </td>
